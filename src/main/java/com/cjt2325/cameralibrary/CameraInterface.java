...
public class CameraInterface implements Camera.PreviewCallback {
    ...
    private static final int VIDEO_BIT_RATE_LOW = 1;// 低码率为视频分辨率*1
    private static final int VIDEO_BIT_RATE_MID = 2;// 中等码率为视频分辨率*2
    private static final int VIDEO_BIT_RATE_HIGH = 4;// 高码率为视频分辨率*4
    private static final int VIDEO_BIT_RATE_RATIO = VIDEO_BIT_RATE_MID;

    private static final int JPEG_QUALITY = 100;// 照片质量，满分100
    private static final int PREVIEW_MAX_WIDTH = 1200;// 竖屏下预览最大宽度
    private static final int PHOTO_MAX_WIDTH = 1600;// 竖屏下拍照最大宽度（nexus 6p 没有1440尺寸）
    private static final int VIDEO_MAX_WIDTH = 800;// 竖屏下录像最大宽度（理想状态是720）
    ...
    private void rotationAnimation() {
        ...
        if (rotation != angle) {
            ...
            ObjectAnimator animC = ObjectAnimator.ofFloat(mSwitchView, "rotation", start_rotation, end_rotation);
            ObjectAnimator animF = ObjectAnimator.ofFloat(mFlashLamp, "rotation", start_rotation, end_rotation);
...
//                Camera.Size previewSize = CameraParamUtil.getInstance().getOptimalSize(
//                        mParams.getSupportedPreviewSizes(),
//                        mHolder.getSurfaceFrame().width(),
//                        mHolder.getSurfaceFrame().height(),
//                        PREVIEW_MAX_WIDTH);// 预览最大宽度设为1200（节省内存）
//                Camera.Size pictureSize = CameraParamUtil.getInstance().getOptimalSize(
//                        mParams.getSupportedPictureSizes(),
//                        mHolder.getSurfaceFrame().width(),
//                        mHolder.getSurfaceFrame().height(),
//                        PHOTO_MAX_WIDTH);// 照片最大宽度设为1600（控制照片大小）

                Camera.Size previewSize = CameraParamUtil.getInstance().getPreviewSize(mParams
                        .getSupportedPreviewSizes(), 1000, screenProp);
                Camera.Size pictureSize = CameraParamUtil.getInstance().getPictureSize(mParams
                        .getSupportedPictureSizes(), 1800, screenProp);// th是指长边最小值
                ...
                if (CameraParamUtil.getInstance().isSupportedPictureFormats(mParams.getSupportedPictureFormats(),
                    ...
                    mParams.setJpegQuality(JPEG_QUALITY);
                }
                mCamera.setParameters(mParams);
                mParams = mCamera.getParameters();
    ...
    void doDestroyCamera() {
        ...
        if (null != mCamera) {
            try {
                mCamera.setPreviewCallback(null);
                // 这两句置空会导致切换摄像头后rotateAnimation()方法失效
                //mSwitchView = null;
                //mFlashLamp = null;
                mCamera.stopPreview();
                //这句要在stopPreview后执行，不然会卡顿或者花屏
...
//
        ...
        LogUtil.e(TAG, "TakePic start:" + startTime);
        try {
            mCamera.takePicture(null, null, new Camera.PictureCallback() {
                ...
                public void onPictureTaken(byte[] data, Camera camera) {
                    ...
                    LogUtil.e(TAG, "TakePic end:" + (System.currentTimeMillis() - startTime));
                    if (callback != null) {
    ...
    public void startRecord(Surface surface, float screenProp, ErrorCallback callback) {
        ...
        yuv.compressToJpeg(new Rect(0, 0, width, height), 60, out);
        ...
        if (mParams.getSupportedVideoSizes() == null) {
            videoSize = CameraParamUtil.getInstance().getOptimalSize2(
                    mParams.getSupportedPreviewSizes(),
                    mHolder.getSurfaceFrame().width(),
                    mHolder.getSurfaceFrame().height(),
                    PREVIEW_MAX_WIDTH);
//            videoSize = CameraParamUtil.getInstance().getPreviewSize(mParams.getSupportedPreviewSizes(), 600, screenProp);
            ...
            videoSize = CameraParamUtil.getInstance().getOptimalSize2(
                    mParams.getSupportedVideoSizes(),
                    mHolder.getSurfaceFrame().width(),
                    mHolder.getSurfaceFrame().height(),
                    VIDEO_MAX_WIDTH);
...
//        }
        ...
        int bitRate = videoSize.width * videoSize.height;
        if (bitRate <= 0) {
            bitRate = preview_width * preview_height;
        }
        bitRate = bitRate * VIDEO_BIT_RATE_RATIO;

        if (DeviceUtil.isHuaWeiRongyao()) {
            // 华为荣耀为什么需要单独设置（该比特率太小了）
    ...
    public void setFlashVisible() {
        setFlashVisible(mParams);
    }

    private void setFlashVisible(Camera.Parameters params) {
        if (params != null && mFlashLamp != null) {
            Activity activity=getActivityFromView(mFlashLamp);
            if(activity!=null&&!activity.isDestroyed()){
                if (params.getSupportedFlashModes() == null || params.getSupportedFlashModes().isEmpty()) {
